...
fn calculate_neighboring_depth_differences(pixel_coordinates: vec2<i32>) -> f32 {
    ...
    let depths_upper_left = textureGather(0, depth_texture, point_clamp_sampler, uv);
    let depths_bottom_right = textureGather(0, depth_texture, point_clamp_sampler, uv, vec2<i32>(1i, 1i));
...
fn load_and_reconstruct_view_space_position(uv: vec2<f32>, sample_mip_level: f32) -> vec3<f32> {
    let depth = textureSampleLevel(depth_texture, linear_clamp_sampler, uv, sample_mip_level).r;
